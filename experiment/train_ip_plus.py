...
from transformers import CLIPImageProcessor, AutoImageProcessor, AutoModel
...
class DINOImageEncoder(torch.nn.Module):
    def __init__(self, modelpath, *args, **kwargs):
        ...
        self.dinov2 = AutoModel.from_pretrained(modelpath)
        self.projection = torch.nn.Linear(self.dinov2.config.hidden_size, 1280, bias=False) # 1280 aims to adapt with hidden size of Open CLIP model
    
    def forward(self, x):
        x = self.dinov2(x)
        x = self.projection(x.last_hidden_state)
        return x

# issue: https://github.com/eriklindernoren/PyTorch-YOLOv3/issues/162
...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, json_file, tokenizer, size=512, t_drop_rate=0.05, i_drop_rate=0.05, ti_drop_rate=0.05, cloth_root_path="", person_root_path=""):
        super().__init__()
        ...
        self.cloth_root_path = cloth_root_path
        self.person_root_path = person_root_path

        # list of dict: [{'cloth_file': 'cloth_1.jpg', 'person_file': 'person_1.jpg', 'text': ''}]
        ...
        # self.dinov2_image_processor = AutoImageProcessor.from_pretrained('facebook/dinov2-giant')
        self.clip_image_processor = CLIPImageProcessor()

    def __getitem__(self, idx):
        item = self.data[idx] 
        text = item["text"]
        person_image = item["person_file"]
        ip_image = item["cloth_file"]
        
        # read image
        ip_image = Image.open(os.path.join(self.cloth_root_path, ip_image))
        # ip_image = self.dinov2_image_processor(ip_image, return_tensors='pt').pixel_values
        ip_image = self.clip_image_processor(ip_image, return_tensors='pt').pixel_values

        person_image = Image.open(os.path.join(self.person_root_path, person_image))
        person_image = self.transform(person_image.convert('RGB'))
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="",
        required=True,
        help="Training data cloth root path",
    )
    parser.add_argument(
        "--person_root_path",
        type=str,
        default="",
        required=True,
        help="Training data person root path",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Gradient accumulation steps to increase batch size for limited memory."
    )
    parser.add_argument(
...
def main():
    ...
    train_dataset = MyDataset(
        ...
        cloth_root_path=args.cloth_root_path,
        person_root_path=args.person_root_path
    )
    ...
    for epoch in range(0, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(ip_adapter):
                ...
                with torch.no_grad():
                    image_embeds = image_encoder(clip_images.to(accelerator.device, dtype=weight_dtype), output_hidden_states=True).last_hidden_state
            ...
            if global_step % args.save_steps == 0:
                ...
                accelerator.save_state(save_path, safe_serialization=False) # https://github.com/huggingface/transformers/issues/27293#issuecomment-1986784382
