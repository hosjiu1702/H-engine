...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default='adapter/ip-adapter-plus_sd15.bin',
        help="Path to pretrained model or model identifier from huggingface.co/models.",
    )
...
class TryOnDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #print(index)
        mask = Image.open(self.agnostic_path[index])
...
def main():
    ...
    # ref_unet = UNet2DConditionModel.from_pretrained("SG161222/Realistic_Vision_V4.0_noVAE", subfolder="unet")
    ...
    # # Unfreeze the parameters of the new convolutional layer
    ...
    #params_to_opt = itertools.chain(unet.conv_in.parameters())
    ...
    dataset = TryOnDataset(args.dataset_json_path, tokenizer=tokenizer)
    ...
    #unet, image_proj, adapter_modules, optimizer, lr_scheduler = accelerator.prepare(unet, image_proj, adapter_modules, optimizer, lr_scheduler)
        ...
        for batch in train_dataloader:
            ...
            with torch.no_grad():
                ...
                ref_latents = vae.encode(
                    batch["cloth_unet_pixel_values"].to(accelerator.device, dtype=weight_dtype)).latent_dist.sample()
                ref_latents = ref_latents * 0.18215
                
                
                masked_latents = vae.encode(
                    batch["masked_images"].reshape(batch["pixel_values"].shape).to(accelerator.device,dtype=weight_dtype)
                ).latent_dist.sample()
                ...
                mask = mask.reshape(-1, 1, 512 // 8, 512 // 8).to('cuda')
            ...
            ### add more latents dim:
            #print(noisy_latents.device, mask.device, masked_latents.device, ref_latents.device)
            noisy_latents = torch.cat([noisy_latents, mask, masked_latents, ref_latents], dim=1)

            clip_images = []
            for clip_image in batch["cloth_embeds"]:
            ...
            with torch.no_grad():
                ...
                #image_proj.to(accelerator.device)
                ip_tokens = image_proj(image_embeds.to(accelerator.device))
            ...
            if isinstance(checkpointing_steps, int):
                if global_steps % checkpointing_steps == 0:
                    ...
                    torch.save(unet.state_dict(), "sd_model_state.pth")
                    torch.save(image_proj.state_dict(), "img_proj.pth")
