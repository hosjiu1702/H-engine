from dressing_sd.pipelines.IMAGDressing_v1_pipeline_controlnet_inpainting_without_ref import IMAGDressing_v1
import os
import torch
...
def prepare(args):
    ...
    vae = AutoencoderKL.from_pretrained("stabilityai/sd-vae-ft-mse").to(device=args.device)
    tokenizer = CLIPTokenizer.from_pretrained("botp/stable-diffusion-v1-5-inpainting", subfolder="tokenizer")
    text_encoder = CLIPTextModel.from_pretrained("botp/stable-diffusion-v1-5-inpainting", subfolder="text_encoder").to(device=args.device)
    image_encoder = CLIPVisionModelWithProjection.from_pretrained("h94/IP-Adapter", subfolder="models/image_encoder").to(device=args.device)
    unet = UNet2DConditionModel.from_pretrained("botp/stable-diffusion-v1-5-inpainting", subfolder="unet").to(device=args.device)
    ...
    image_proj = image_proj.to(device=args.device)
    ...
    adapter_modules = adapter_modules.to(device=args.device)
    del st
...
#     adapter_modules.load_state_dict(adapter_modules_dict, strict=False)
    unet.load_state_dict(torch.load('sd_model_state.pth'))
    #image_proj.load_state_dict(torch.load('img_proj.pth'))
    ...
    control_net = ControlNetModel.from_pretrained("lllyasviel/control_v11p_sd15_inpaint").to(device=args.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--masked_image', type=str, required=True)
    ...
    validation_mask = Image.open(args.masked_image).convert("L")
