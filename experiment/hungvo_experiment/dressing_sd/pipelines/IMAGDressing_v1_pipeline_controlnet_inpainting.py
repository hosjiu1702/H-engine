...
class IMAGDressing_v1(StableDiffusionControlNetInpaintPipeline):
    ...
    def __call__(
        ...
        ref_image_latents_org =  ref_image_latents
        ref_image_latents_org = torch.cat([ref_image_latents_org] * 2) if self.do_classifier_free_guidance else ref_image_latents_org
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if num_channels_unet == 13:
                    #print(ref_image_latents_org.shape, latent_model_input.shape,mask.shape,'------------------------------------------------------------------------------')    
                    
                    latent_model_input = torch.cat([latent_model_input, mask, masked_image_latents, ref_image_latents_org], dim=1)
