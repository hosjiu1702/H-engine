...
class UNet2DConditionModelRT(UNet2DConditionModel):    
    ...
    def forward(
        ...
        down_block_1: torch.Tensor,
        down_block_2: torch.Tensor,
        down_block_3: torch.Tensor,
        down_block_4: torch.Tensor,
        down_block_5: torch.Tensor,
        down_block_6: torch.Tensor,
        down_block_7: torch.Tensor,
        down_block_8: torch.Tensor,
        down_block_9: torch.Tensor,
        down_block_10: torch.Tensor,
        down_block_11: torch.Tensor,
        down_block_12: torch.Tensor,
        ...
        down_block_additional_residuals = [down_block_1, down_block_2, down_block_3, down_block_4, down_block_5, down_block_6, down_block_7, down_block_8, down_block_9, down_block_10, down_block_11, down_block_12]
        
        # down_block_additional_residuals = [torch.cat([torch.zeros_like(d), d]) for d in down_block_additional_residuals]
        # print(sample.shape, encoder_hidden_states.shape)
        # for i in range(len(down_block_additional_residuals)):
        #    print(down_block_additional_residuals[i].shape)
...
#         down_samples, mid_sample = self.controlnets(
#                 org_latents,
#                 timestep,
#                 encoder_hidden_states=encoder_hidden_states,
...
#             onnx.save_model(
#             onnx_graph,
#             "save_trash/temp_model.onnx",
#             save_as_external_data=True,
#             all_tensors_to_one_file=False,
#             location="external_data.bin",
#             size_threshold=1024,
#             )
#             onnx_graph = onnx.load("save_trash/temp_model.onnx")
...
#         #else:
        ...
        if onnx_graph.ByteSize() > 2147483648:
            print(onnx_graph.ByteSize())
            raise TypeError("ERROR: model size exceeds supported 2GB limit")
            # from onnx.external_data_helper import convert_model_to_external_data
            # convert_model_to_external_data(onnx_graph, all_tensors_to_one_file=True, location="weights.bin")
            # onnx.save_model(onnx_graph, "model_with_external_data.onnx")
            # print("Model saved with external data.")
        else:
            onnx_graph = shape_inference.infer_shapes(onnx_graph)
...
class UNet(BaseModel):
    def __init__(
        self, model, fp16=True, device="cuda", max_batch_size=16, 
        embedding_dim=768, text_maxlen=81, unet_dim=4
    ):
        super().__init__(
            model=model,
            fp16=fp16,
    ...
    def get_input_names(self):
        return [
            ...
            "down_block_1", "down_block_2", "down_block_3", "down_block_4",
            "down_block_5", "down_block_6", "down_block_7", "down_block_8",
            "down_block_9", "down_block_10", "down_block_11", "down_block_12",
    ...
    def get_dynamic_axes(self):
        return {
            ...
            "latent": {0: "2B", 2: "H", 3: "W"},
            **{f"down_block_{i}": {0: "2B"} for i in range(1, 13)}, 
            "mid_block_additional_residual": {0: "2B"},
        }
    ...
    def get_input_profile(self, batch_size, image_height, image_width, static_batch, static_shape):
        ...
        320, 640, 640,  # (64, 48)
        640, 1280, 1280,  # (32, 24)
        1280, 1280, 1280  # (16, 12)
    ]

    # Define fixed resolutions based on your example
        down_block_resolutions = [
        (128, 96),  # down_block_1
        (128, 96),  # down_block_2
        (128, 96),  # down_block_3
        (64, 48),  # down_block_4
        ...
        input_profile = {
        "sample": [
            (2 * min_batch, self.unet_dim, min_latent_height, min_latent_width),  # Min shape
            (2 * batch_size, self.unet_dim, latent_height, latent_width),         # Optimal shape
            (2 * max_batch, self.unet_dim, max_latent_height, max_latent_width),  # Max shape
        ],
        "encoder_hidden_states": [
            (2 * min_batch, self.text_maxlen, self.embedding_dim),  # Min shape
            (2 * batch_size, self.text_maxlen, self.embedding_dim), # Optimal shape
            (2 * max_batch, self.text_maxlen, self.embedding_dim),  # Max shape
        ],
        # "latent": [
        #     (2 * min_batch, 4, min_latent_height, min_latent_width),  # Min shape
        #     (2 * batch_size, 4, latent_height, latent_width),         # Optimal shape
        #     (2 * max_batch, 4, max_latent_height, max_latent_width),  # Max shape
        # ],
    }
        for i in range(12):  # Assuming 12 down blocks
        #down_block_channels = self.get_down_block_channels()  # Define this method to get channel sizes
        #down_block_resolutions = self.get_down_block_resolutions()  # Define this method to get resolutions

            input_profile[f"down_block_{i+1}"] = [
            (2 * min_batch, down_block_channels[i], *down_block_resolutions[i]),  # Min shape
            (2 * batch_size, down_block_channels[i], *down_block_resolutions[i]), # Optimal shape
            (2 * max_batch, down_block_channels[i], *down_block_resolutions[i]),  # Max shape
        ]

    # Add mid_block shape
        input_profile["mid_block_additional_residual"] = [
        (2 * min_batch, 1280, 16, 12),  # Min shape
        (2 * batch_size, 1280, 16, 12), # Optimal shape
        (2 * max_batch, 1280, 16, 12),  # Max shape
    ]
    ...
    # Define fixed channel sizes based on your example
        down_block_channels = [
        320, 320, 320,  # (128, 96)
        320, 640, 640,  # (64, 48)
        640, 1280, 1280,  # (32, 24)
    ...
    # Generate shape dictionary
        ...
        **{
            f"down_block_{i+1}": (2 * batch_size, down_block_channels[i], *down_block_resolutions[i])
            for i in range(12)
        },
        "mid_block_additional_residual": (2 * batch_size, 1280, 16, 12)
    ...
    # Define fixed channel sizes for each down block
        down_block_channels = [
        320, 320, 320,   # down_block_1 to down_block_3
        320, 640, 640,   # down_block_4 to down_block_6
        640, 1280, 1280, # down_block_7 to down_block_9
        1280, 1280, 1280 # down_block_10 to down_block_12
    ]

    # Define fixed resolutions for each down block
        down_block_resolutions = [
        (128, 96), (128, 96), (128, 96),  # down_block_1 to down_block_3
        (64, 48), (64, 48), (64, 48),  # down_block_4 to down_block_6
        (32, 24), (32, 24), (32, 24),  # down_block_7 to down_block_9
        (16, 12), (16, 12), (16, 12)      # down_block_10 to down_block_12
    ]

    # Generate down_block residual tensors
        down_block_residuals = [
        torch.randn(2 * batch_size, down_block_channels[i], *down_block_resolutions[i], dtype=dtype, device=self.device)
        for i in range(12)
        ]

        return (
        torch.randn(2 * batch_size, self.unet_dim, latent_height, latent_width, dtype=dtype, device=self.device),  # Sample input
        torch.tensor([1.0], dtype=torch.float32, device=self.device),  # Timestep tensor
        torch.randn(2 * batch_size, self.text_maxlen, self.embedding_dim, dtype=dtype, device=self.device),  # Encoder hidden states
        *down_block_residuals,  # Unpacking all down block tensors
        torch.randn(2 * batch_size, 1280, 16, 12, dtype=dtype, device=self.device)
    )

# class UNet(BaseModel):
#     def __init__(
#         self, model, fp16=True, device="cuda", max_batch_size=16, embedding_dim=768, text_maxlen=81, unet_dim=9
#     ):
...
#     def get_input_names(self):
#         #return ["sample", "org_latents", "timestep", "encoder_hidden_states", "controlnet_conds", "controlnet_scales"]
#         return ["sample", "org_latents", "timestep", "encoder_hidden_states", "controlnet_conds"]
...
#         return {
#             "sample": {0: "2B", 2: "H", 3: "W"},
#             #"org_latents": {0: "2B", 2: "H", 3: "W"},
#             "encoder_hidden_states": {0: "2B"},
#             #"controlnet_conds": {0: "2B", 2: "8H", 3: "8W"},
#             "controlnet_conds": {0: "2B"},
#             "latent": {0: "2B", 2: "H", 3: "W"},
#         }
...
#             ],
#             "org_latents": [
#                 (2 * min_batch, 4, min_latent_height, min_latent_width),
#                 (2 * batch_size, 4, latent_height, latent_width),
#                 (2 * max_batch, 4, max_latent_height, max_latent_width),
#             ],
#             "encoder_hidden_states": [
...
#             ],
#             "controlnet_conds": [
#                 (2 * min_batch, 3, min_latent_height * 8, min_latent_width * 8),
#                 (2 * min_batch, 3, min_latent_height * 8, min_latent_width * 8),
#                 (2 * max_batch, 3, max_latent_height * 8, max_latent_width * 8),
#             ],
#         }
...
#         return {
#             "sample": (2 * batch_size, self.unet_dim, latent_height, latent_width),
#             "org_latents": (2 * batch_size, 4, latent_height, latent_width),
#             "encoder_hidden_states": (2 * batch_size, self.text_maxlen, self.embedding_dim),
#             "controlnet_conds": (2 * batch_size, 3, latent_height * 8, latent_width * 8),
#             "latent": (2 * batch_size, 4, latent_height, latent_width),
#         }
...
#         #["sample", "input_latents", "timestep", "controlnet_conds", "encoder_hidden_states"]
#         #["sample", "timestep", "encoder_hidden_states", "controlnet_conds", "controlnet_scales"]
...
#             ),
#             torch.randn(
#                 2 * batch_size, 4, latent_height, latent_width, dtype=torch.float32, device=self.device
#             ),
#             torch.tensor([1.0], dtype=torch.float32, device=self.device),
#             torch.randn(2 * batch_size, self.text_maxlen, self.embedding_dim, dtype=torch.float32, device=self.device),
#             torch.randn(2, 3, image_height, image_width, dtype=torch.float32, device=self.device),
...
def make_UNet(model, device="cuda", max_batch_size = 16, embedding_dim = 768, inpaint=False, unet_dim=9):
    return UNet(
        model,
        fp16=True,
        device=device,
...
class TensorRTStableDiffusionInpaintPipeline(DiffusionPipeline):
    ...
    def __loadModels(self):
        ...
        if "unet" in self.stages:
            #print('------------------------------------------------------------')
            self.models["unet"] = make_UNet(self.unet)
    ...
    def __encode_prompt(self, prompt, negative_prompt):
        ...
        return text_embeddings.to(dtype=torch.float32), uncond_embeddings.to(dtype=torch.float32)
    ...
    ,control_image = None):
        ...
        with tqdm(total=self.denoising_steps) as progress_bar:
            ...
            # Predict the noise residual
                ...
                {"sample": latent_model_input, 
                 ...
                 "down_block_1": down_block_res_samples[0],
                 "down_block_2": down_block_res_samples[1],
                 "down_block_3": down_block_res_samples[2],
                 "down_block_4": down_block_res_samples[3],
                 "down_block_5": down_block_res_samples[4],
                 "down_block_6": down_block_res_samples[5],
                 "down_block_7": down_block_res_samples[6],
                 "down_block_8": down_block_res_samples[7],
                 "down_block_9": down_block_res_samples[8],
                 "down_block_10": down_block_res_samples[9],
                 "down_block_11": down_block_res_samples[10],
                 "down_block_12": down_block_res_samples[11],
                 "mid_block_additional_residual": mid_block_res_sample,
                },
                self.stream,
                )["latent"]
    ...
    def __call__(
        ...
        image_prompt_embeds: Union[torch.Tensor, PIL.Image.Image] = None,
        image: Union[torch.Tensor, PIL.Image.Image] = None,
        mask_image: Union[torch.Tensor, PIL.Image.Image] = None,
        strength: float = 1.00,
        num_inference_steps: int = 100,
        guidance_scale: float = 7.5,
        image_negative_prompt_embeds: Union[torch.Tensor, PIL.Image.Image] = None,
        generator: Optional[Union[torch.Generator, List[torch.Generator]]] = None,
        control_image: Union[torch.Tensor, PIL.Image.Image] = None
    ):
        r"""
...
#         assert len(prompt) == len(negative_prompt)
        ...
        with torch.inference_mode(), torch.autocast("cuda"), trt.Runtime(TRT_LOGGER):
            ...
            # CLIP text encoder
            text_embeddings, negative_text_embeddings = self.__encode_prompt(prompt, negative_prompt)
            
            
            text_embeddings = torch.cat([text_embeddings, image_prompt_embeds], dim=1)
            negative_text_embeddings = torch.cat([negative_text_embeddings, image_negative_prompt_embeds], dim=1)
