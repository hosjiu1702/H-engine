...
class MatrixVisualizer:
    ...
    def visualize(self, image_bgr, mask, matrix, bbox_xywh):
        ...
        image_target_bgr[y : y + h, x : x + w, :] = np.where(mask_bg, image_target_bgr[y : y + h, x : x + w, :] * 0, image_target_bgr[y : y + h, x : x + w, :])
        image_target_bgr[:, :x, :] *= 0
        image_target_bgr[:y, :, :] *= 0
        image_target_bgr[:, x+w:, :] *= 0
        image_target_bgr[y+h:, :, :] *= 0

        return image_target_bgr.astype(np.uint8), matrix_vis, mask_bg
