...
class DensePoseResultsVisualizer:
    def visualize(
        ...
        for i, result in enumerate(densepose_result):
            ...
            matrix_bgr, mask_bg, mask = self.visualize_iuv_arr(context, iuv_array.cpu().numpy(), boxes_xywh[i])
        image_bgr = self.context_to_image_bgr(context)
        return image_bgr, matrix_bgr, mask_bg, mask
...
class DensePoseMaskedColormapResultsVisualizer(DensePoseResultsVisualizer):
    ...
    def visualize_iuv_arr(self, context, iuv_arr: np.ndarray, bbox_xywh):
        ...
        mask[segm > 0] = 1
        image_bgr, matrix_vis, mask_bg = self.mask_visualizer.visualize(image_bgr, mask, matrix, bbox_xywh)
        return matrix_vis, mask_bg, mask
...
class DensePoseResultsFineSegmentationVisualizer(DensePoseMaskedColormapResultsVisualizer):
    def __init__(self, inplace=True, cmap=cv2.COLORMAP_PARULA, alpha=1.0, **kwargs):
