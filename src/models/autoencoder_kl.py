...
class AutoencoderKL(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def encode(
        ...
        if self.use_slicing:
            raise ValueError('Do not support slicing for vae because of EMASC module from LaDI-VTON paper.')
        ...
        posterior = DiagonalGaussianDistribution(moments) # Should care more about this Distribution in the near future.
    ...
    def _decode(
        ...
        if intermediate_features:
            dec = self.decoder(z, intermediate_features, emasc_layers)
        else:
            dec = self.decoder(z)
    ...
    def decode(
        ...
        if self.use_slicing and z.shape[0] > 1:
            ...
            if intermediate_features:
                decoded = self._decode(z, intermediate_features, emasc_layers).sample
            else:
