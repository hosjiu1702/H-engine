...
class UNet(BaseModel):
    def __init__(
        self, model, fp16=False, device="cuda", max_batch_size=2, embedding_dim=768, text_maxlen=81, unet_dim=9
    ):
    ...
    def get_input_profile(self, batch_size, image_height, image_width, static_batch, static_shape):
        ...
        return {
            "sample": [
                (2 * min_batch, self.unet_dim, min_latent_height * 2, min_latent_width),
                (2 * batch_size, self.unet_dim, latent_height * 2, latent_width),
                (2 * max_batch, self.unet_dim, max_latent_height * 2, max_latent_width),
            ],
    ...
    def get_shape_dict(self, batch_size, image_height, image_width):
        latent_height, latent_width = self.check_dims(batch_size, image_height, image_width)
        return {
            "sample": (2 * batch_size, self.unet_dim, latent_height * 2, latent_width),
            #"encoder_hidden_states": (2 * batch_size, self.text_maxlen, self.embedding_dim),
            "latent": (2 * batch_size, 4, latent_height * 2, latent_width),
        }
    ...
    def get_sample_input(self, batch_size, image_height, image_width):
        ...
        return (
            torch.randn(
                2 * batch_size, self.unet_dim, latent_height * 2, latent_width, dtype=torch.float32, device=self.device
            ),
...
class TryOnPipeline(
    ...
    def prepare_ip_adapter_image_embeds(
        ...
        if ip_adapter_image_embeds is None:
            ...
            if do_classifier_free_guidance:
                ...
                image_embeds = single_negative_image_embeds
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # 1. Check inputs
        # we should convert image type from 
        self.check_inputs(
            image,
            mask_image,
            height,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = torch.cat([latent_model_input, mask_concat, masked_image_latents_concat, densepose_latents_concat], dim=1)
